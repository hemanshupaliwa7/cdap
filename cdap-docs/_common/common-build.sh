...
function build_license_pdfs() {
  ...
  PDFS="cdap-enterprise-dependencies cdap-level-1-dependencies cdap-sandbox-dependencies cdap-ui-dependencies"
  for PDF in ${PDFS}; do
    echo
    echo "Building ${PDF}"
...
function display_version() {
  ...
  echo "  GIT_BRANCH_CDAP_CLIENTS: ${GIT_BRANCH_CDAP_CLIENTS}"
  echo "  GIT_BRANCH_CDAP_GUIDES: ${GIT_BRANCH_CDAP_GUIDES}"
  echo "  GIT_BRANCH_CDAP_INGEST: ${GIT_BRANCH_CDAP_INGEST}"
  echo "  GIT_BRANCH_CDAP_PACKS: ${GIT_BRANCH_CDAP_PACKS}"
  echo "CDAP Apps:"
